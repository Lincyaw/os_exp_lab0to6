...
fn test_heap_alloc_and_free() {
    ...
    let space: [usize; 128] = [0; 128];
    unsafe {
        heap.add_to_heap(space.as_ptr() as usize, space.as_ptr().add(128) as usize);
    }
    let addr0 = heap.alloc(Layout::from_size_align(15, 8).unwrap()).unwrap();
    let addr1 = heap.alloc(Layout::from_size_align(15, 8).unwrap()).unwrap();
    let addr2 = heap.alloc(Layout::from_size_align(15, 8).unwrap()).unwrap();
    let addr3 = heap.alloc(Layout::from_size_align(15, 8).unwrap()).unwrap();
    heap.dealloc(addr0, Layout::from_size_align(15, 8).unwrap());
    heap.dealloc(addr1, Layout::from_size_align(15, 8).unwrap());
    heap.dealloc(addr2, Layout::from_size_align(15, 8).unwrap());
    heap.dealloc(addr3, Layout::from_size_align(15, 8).unwrap());
