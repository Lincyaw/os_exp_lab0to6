...
impl FrameAllocator {
    ...
    pub fn add_frame(&mut self, start: usize, end: usize) {
        ...
        while current_start < end {     //如果没有超出end的话
            let lowbit = if current_start > 0 {  //令lowbit=从右到左出现1的数
    ...
    pub fn alloc(&mut self, count: usize) -> Option<usize> {
        //假设count=12
        let size = count.next_power_of_two(); //得到最小的 大于count的2的n次方
        //size=16
        let class = size.trailing_zeros() as usize;  //返回size中尾随0的个数
        //class=4
        for i in class..self.free_list.len() {  //遍历class到整个freelist的大小
            // for i in 4..32
            // Find the first non-empty size class
            if !self.free_list[i].is_empty() {  //如果第i个不为空
                // Split buffers
                for j in (class + 1..i + 1).rev() {   //从i+1到class+1
                    // for j in i+1..5
                    if let Some(block_ref) = self.free_list[j].iter().next() {  //如果第j-1块没问题的话
                        let block = *block_ref;
                        self.free_list[j - 1].insert(block + (1 << (j - 1))); //第j-1块就插入 第j-1块的值加...
                        self.free_list[j - 1].insert(block);                  //也插入第j-1块
